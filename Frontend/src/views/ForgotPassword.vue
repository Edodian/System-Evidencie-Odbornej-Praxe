  ...
  <div class="min-h-screen flex items-center justify-center bg-gray-50 px-4">
    <div class="w-full max-w-md bg-white rounded-2xl shadow-lg p-8 relative">
      ...
      <p class="text-gray-600 text-center mb-6 text-sm">
        Enter your email address and we will send you a temporary code to reset your password.
      </p>
  ...
  try {
    ...
    if (err.response) {
      if (err.response.status === 404) {
        ...
        error.value = err.response.data?.error || "Server error. Please try again later."
      }
    } else {
